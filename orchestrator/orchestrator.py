...
class IncidentOrchestrator:
    ...
    def analyze_incident(self, events: List[Dict], save_intermediates: bool = True) -> Dict:
        ...
        print("=" * 70)
        print("🔍 SECURITY INCIDENT ANALYSIS SYSTEM")
        print("=" * 70)
        ...
        if not attack_analysis.get('attack_detected'):
            print("✓ No attacks detected. System is secure.")
            return self._generate_clean_report(events)

        print(f"✓ Detected {attack_analysis['total_attacks']} attack pattern(s)")
        ...
        print(f"✓ Estimated financial impact: ${total_loss:,} USD")
        ...
        print(f"✓ Generated comprehensive response plan")
    ...
    def _generate_final_report(self, events: List[Dict], attack_analysis: Dict,
        ...
        return {
            ...
            "severity_assessment": {
                ...
                "severity_justification": self._generate_severity_justification(attack_analysis, impact_analysis)
            },
            "attack_intelligence": {
            ...
            "business_impact": {
                ...
                "affected_asset_summary": impact_analysis.get('affected_asset_summary', []),
                "compliance_violations": impact_analysis.get('compliance_violations', []),
                "requires_notification": impact_analysis.get('requires_notification', False)
            },
            "incident_response": {
                ...
                "notification_requirements": response_plan.get('notification_requirements', [])
            },
            "recommendations": self._generate_recommendations(attack_analysis, impact_analysis),
            "analysis_metadata": {
                "total_events_analyzed": len(events),
                "malicious_events": len([e for e in events if e.get('is_malicious')]),
                "benign_events": len([e for e in events if not e.get('is_malicious')]),
                ...
                "agents_used": ["Attack Tracer", "Impact Analyst", "Incident Responder"]
            }
        }
    ...
    def _generate_severity_justification(self, attack_analysis: Dict, impact_analysis: Dict) -> str:
        severity = impact_analysis.get('overall_severity', 'Unknown')
        num_attacks = attack_analysis.get('total_attacks', 0)
        financial_loss = impact_analysis.get('total_estimated_loss_usd', 0)

        justification = f"Severity rated as {severity} based on: "

        reasons = []
        if num_attacks > 5:
            reasons.append(f"multiple concurrent attacks ({num_attacks} detected)")
        if financial_loss > 50000:
            reasons.append(f"significant financial impact (${financial_loss:,})")
        if impact_analysis.get('requires_notification'):
            reasons.append("regulatory notification requirements")
        if impact_analysis.get('total_affected_assets', 0) > 3:
            reasons.append("multiple critical assets affected")

        justification += ", ".join(reasons) if reasons else "attack characteristics and potential impact"

        return justification + "."

    def _generate_recommendations(self, attack_analysis: Dict, impact_analysis: Dict) -> List[Dict]:
        recommendations = []
        ...
        if severity in ['Critical', 'High']:
            recommendations.append({
                ...
                "recommendation": "Activate incident response team immediately",
                "rationale": f"Severity level is {severity}"
            })

        attack_types = set(chain['attack_type'] for chain in attack_analysis.get('attack_chains', []))
    ...
    def save_report(self, report: Dict, filepath: str = None):
        ...
        if filepath is None:
            filepath = PROJECT_ROOT / f"data/reports/incident_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        else:
...
def main():
    ...
    try:
        ...
        print("Please run parser.py first.")
        return
